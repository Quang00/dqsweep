...
from experiments.utils import (
    compute_fidelity,
    distributed_U_control,
    distributed_U_target,
    toffoli,
)
...
class AliceToffoli(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        for _ in range(self._num_epr_rounds):
            ...
            a_q.X()
            yield from distributed_U_control(context, self.PEER_NAME, a_q)
            a_q.measure()
...
class BobToffoli(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        for _ in range(self._num_epr_rounds):
            ...
            b_q.X()
            yield from distributed_U_control(context, self.PEER_NAME, b_q)
            b_q.measure()
            yield from connection.flush()
...
class CharlieToffoli(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        for _ in range(self._num_epr_rounds):
            ...
            yield from distributed_U_target(context, self.PEERS, c_q, toffoli)
            yield from connection.flush()
